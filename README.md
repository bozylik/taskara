...
taskara is a simple and fast library for managing tasks in go. it allows you to runClusterTask many tasks concurrently using a controlled number of workers. it is designed to be minimal, having zero external dependencies.
...
> [!NOTE]
> If you leave the id empty, a unique getID will be automatically generated by the cluster during the `Submit()` call.<br>

>**TaskInterface (methods)**<br>

These methods are available on a task instance.

`getID() string` - Returns the task's unique identifier.<br>
`SetID(id string)` - Updates the task getID manually.<br>
...
`runClusterTask()` - Starts the cluster's execution engine. Workers begin listening to the queue and processing tasks.<br>
> [!NOTE]
> Tasks will remain in the queue and won't start until runClusterTask() is called.
...
`CancelTask(id string)` - Targets and cancels a specific task by its getID. This triggers the `cancelled` channel inside the `TaskFunc` and closes the task's context.<br>
...
`cancelClusterTask()` - **Immediate shutdown**. Instantly kills all workers and cancels all active task contexts.<br>
> [!CAUTION]
...
`WithStartTime(st time.Time) *clusterTaskBuilder` - Schedules the task to runClusterTask at a specific time. If the time is in the past or `time.Now()`, the task will be executed as soon as a worker is available.<br>
...
`Submit() (string, error)` - The final method in the chain. It validates the task, generates an getID (if empty), and pushes the task into the scheduler.
> [!WARNING]
> Returns an error if a task with the same getID is already running or managed by the cluster.<br>
...
A task is a base struct used within clusters. It contains a unique getID and the function to be executed.<br>
